...
class Gender(object):
    ...
    def __init__(self):
        ...
        self.gender_map = {
            ...
            self.UNKNOWN:set(['unknown', 'na', 'not specified']) ## ask about pedfile format things
        }
...
class Person(object):
    ...
    def add_child(self, child):
        ...
        Args:
            self, child
        Raises:
            :obj:`PersonError`: if `child` already has a mother
            :obj:`PersonError`: if child already has a father
            :obj:`PersonError`: if child already self's child
            :obj:`PersonError`: if child is ancestor
            :obj:`personError`: if self does not have a defined gender
        """
        if child.mother != None and self.gender == Gender.FEMALE:
            raise PersonError("'{}' already has a mother and it is '{}'".format(child.name, child.mother.name))
        if child.father != None and self.gender == Gender.MALE:
            raise PersonError("'{}' already has a father and it is '{}'".format(child.name, child.mother.name))
        if child in self.all_ancestors():
            raise PersonError("'{}' is already and ancestor of '{}'", format(child.name, self.name))
        if self.gender == Gender.MALE:
            ...
            raise PersonError("cannot add child with parent with unknown gender")
            return None
                              
        
    def remove_mother(self):
        """ Removes the mother of this person
        Args:
            self
        Raises:
            :obj:`PersonError`: if `mother` doesn't exist or mother has different children
        """
        if self.mother == None:
            raise PersonError("'{}' does not have a set mother".format(self.name))
        if self not in self.mother.children:
            raise PersonError("'{}' is not in '{}' list of children".format(self.name, self.mother.name))
        self.mother.children.remove(self)
        self.mother = None
        return None
        
    def remove_father(self):
        """ Removes the father of this person
        Args:
            self
        Raises:
            :obj:`PersonError`: if `father` doesn't exist or father has different children
        """
        if self.father == None:
            raise PersonError("'{}' does not have a set father".format(self.name))
        if self not in self.father.children:
            raise PersonError("'{}' is not in '{}' list of children".format(self.name, self.father.name))
    ...
    def all_grandparents(self):
        ...
        '''
        ## Fixes to get all of the grandparents had to switch from 3 to 2
